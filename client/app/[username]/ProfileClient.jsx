...
export async function generateStaticParams() {
    try {
        const response = await fetch('https://linktree-backend-hky4.onrender.com/api/profilenames')
...
export function ProfileClient({ initialProfile }) {
    ...
    useEffect(() => {
        ...
        const updateView = async () => {
            try {
                const response = await fetch(`https://linktree-backend-hky4.onrender.com/api/analytics/profile/${profile.id}/view`, {
                    method: "POST",
                    headers: {
