...
export function LinkCollection({ currentProfileId, links, onLinkChange, linkOrder, onLinkOrderChange }) {
    ...
    useEffect(() => {
        const updateOrder = async () => {
            ...
            try {
                await fetch(`https://linktree-backend-hky4.onrender.com/api/links/order`, {
                    method: 'PUT',
                    headers: {
    ...
    const handleEdit = async (id, updates) => {
        if (!currentProfileId) return
        try {
            const response = await fetch(`https://linktree-backend-hky4.onrender.com/api/links/${id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleCreate = async (newLink) => {
        if (!currentProfileId) return
        try {
            const response = await fetch(`https://linktree-backend-hky4.onrender.com/api/links`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
    ...
    const handleDelete = async (id) => {
        if (!currentProfileId) return
        try {
            await fetch(`https://linktree-backend-hky4.onrender.com/api/links/${id}`, {
                method: 'DELETE',
            })
